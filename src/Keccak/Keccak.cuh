...
namespace keccak {
struct KeccakConfig {
    device_context::DeviceContext ctx; /**< Details related to the device such
                                          as its id and stream id. */
    bool are_inputs_on_device;  /**< True if inputs are on device and false if
                                   they're on host. Default value: false. */
    bool are_outputs_on_device; /**< If true, output is preserved on device,
                                   otherwise on host. Default value: false. */
    bool is_async; /**< Whether to run the Keccak asynchronously. If set to
                    * `true`, the keccak_hash function will be non-blocking and
                    * you'd need to synchronize it explicitly by running
                    *   `cudaStreamSynchronize` or `cudaDeviceSynchronize`. If
                    * set to false, keccak_hash function will block the current
                    * CPU thread. */
    static KeccakConfig default_keccak_config() {
        auto ctx = device_context::get_default_device_context();
        return KeccakConfig{ctx, false, false, false};
    }
};
...
__device__ inline void keccakf(uint64_t s[25]) {
